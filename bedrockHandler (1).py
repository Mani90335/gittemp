...
PATCH NOTE (2026-09-15d): _scanCustomModels() and
_scanProvisionedThroughput() now page to exhaustion via get_paginator()
instead of a single direct call. ListCustomModels and
ListProvisionedModelThroughputs are both confirmed real paginated
operations (checked against botocore's own paginators-1.json) — the
single-call version silently under-reported any account with enough
custom models / provisioned throughputs to span multiple pages, with no
error raised. This mirrors the same-class fix already applied to
Comprehend's list_flywheels/list_datasets/list_targeted_sentiment_
detection_jobs in comprehendHandler.py.
...
class BedrockHandler(AIInventoryEventHandlerAWSService):
    ...
    def _scanCustomModels(self):
        """PATCH (2026-09-15d): now pages to exhaustion via
        get_paginator("list_custom_models") instead of one direct call.
        ListCustomModels IS a real paginated operation (confirmed
        against botocore's paginators-1.json for bedrock) — the
        single-call version silently missed any models past the first
        page."""
        logUtils.logInfo(MODULE_NAME, "Inside " + self._scanCustomModels.__name__)
        rows = []
        try:
            for page in self.client.get_paginator("list_custom_models").paginate():
                for m in page.get("modelSummaries", []):
                    try:
                        ...
                        self.logScanError("custom_models:row", e)
        except Exception as e:
    ...
    def _scanProvisionedThroughput(self):
        """PATCH (2026-09-15d): now pages to exhaustion via
        get_paginator("list_provisioned_model_throughputs") — same fix
        and same reasoning as _scanCustomModels() above.
        ListProvisionedModelThroughputs is also a confirmed real
        paginated operation."""
        logUtils.logInfo(MODULE_NAME, "Inside " + self._scanProvisionedThroughput.__name__)
        rows = []
        try:
            for page in self.client.get_paginator("list_provisioned_model_throughputs").paginate():
                for p in page.get("provisionedModelSummaries", []):
                    try:
                        ...
                        self.logScanError("provisioned_throughput:row", e)
        except Exception as e:
